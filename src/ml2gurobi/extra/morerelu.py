...
class ReLUmin():
    ...
    def mip_model(self, layer):
        ...
        if not hasattr(layer, 'mixing'):
            mixing = layer.getModel().addMVar(layer._output.shape, lb=-GRB.INFINITY,
                                              vtype=GRB.CONTINUOUS,
                                              name='mix')
            layer.mixing = mixing
            minact = layer.getModel().addMVar(layer._output.shape, lb=-GRB.INFINITY, ub=0.0,
                                              vtype=GRB.CONTINUOUS,
                                              name='minact')
            layer.minact = minact
...
class GRBReLU():
    ...
    def mip_model(self, layer):
        ...
        if not layer.zvar:
            z = layer.getModel().addMVar(layer._output.shape, ub=1.0, vtype=GRB.BINARY,
                                         name='z')
            layer.zvar = z
            mixing = layer.getModel().addMVar(layer._output.shape, lb=0.0, ub=-layer.wmin, vtype=GRB.CONTINUOUS,
                                              name='mix')
            layer.mixing = mixing
        if self.bigm is not None:
            layer.wmax = np.minimum(layer.wmax, self.bigm)
...
class ReLUM():
    ...
    def mip_model(self, layer):
        ...
        if not layer.zvar:
            ...
            layer.zvar = z
            if not self.expand:
                mixing = layer.getModel().addMVar(layer._output.shape, lb=layer.wmin, ub=layer.wmax, vtype=GRB.CONTINUOUS,
                                                  name='mix')
                layer.mixing = mixing
        if self.bigm is not None:
            layer.wmax = np.minimum(layer.wmax, self.bigm)
        ...
        for index in np.ndindex(layer._output.shape):
            ...
            if ub > self.eps and lb < -self.eps:
                ...
                layer.getModel().addConstr(vact <= mixing - lb * (1 - vz),
        ...
        for index in np.ndindex(layer._output.shape):
            ...
            if self.obbt_rel == 'comb':
                ...
                if abs(ub) > abs(lb):
                    ...
                    layer.getModel().addConstr(vact >= 0, name=constrname+'_low')
            else:
