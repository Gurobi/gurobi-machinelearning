...
class TestONNXDAG(unittest.TestCase):
    ...
    def test_sequential_model(self):
        ...
        graph = helper.make_graph([gemm1, relu1, gemm2], "SequentialMLP", [X], [Y],
    ...
    def test_skip_connection(self):
        ...
        init_W2 = helper.make_tensor("W2", TensorProto.FLOAT, W2.T.shape, W2.T.flatten())
        init_b2 = helper.make_tensor("b2", TensorProto.FLOAT, b2.shape, b2)
        init_W_skip = helper.make_tensor("W_skip", TensorProto.FLOAT, W_skip.T.shape, W_skip.T.flatten())
        init_b_skip = helper.make_tensor("b_skip", TensorProto.FLOAT, b_skip.shape, b_skip)
        ...
        gemm_skip = helper.make_node("Gemm", ["X", "W_skip", "b_skip"], ["branch2"], transB=1)
        
        # Combine branches (residual add)
        add = helper.make_node("Add", ["branch1", "branch2"], ["Y"])
        
        graph = helper.make_graph([gemm1, relu1, gemm2, gemm_skip, add], "SkipConnectionMLP", [X], [Y],
    ...
    def test_residual_connection(self):
        ...
        init_W1 = helper.make_tensor("W1", TensorProto.FLOAT, W1.T.shape, W1.T.flatten())
        init_b1 = helper.make_tensor("b1", TensorProto.FLOAT, b1.shape, b1)
        init_W2a = helper.make_tensor("W2a", TensorProto.FLOAT, W2a.T.shape, W2a.T.flatten())
        init_b2a = helper.make_tensor("b2a", TensorProto.FLOAT, b2a.shape, b2a)
        init_W2b = helper.make_tensor("W2b", TensorProto.FLOAT, W2b.T.shape, W2b.T.flatten())
        ...
        graph = helper.make_graph([gemm1, relu1, gemm2a, gemm2b, add], "ResidualMLP", [X], [Y],
