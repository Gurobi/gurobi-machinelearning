#!/bin/bash
#
# Batch convert all benchmarks to MPS files
#
# Features:
# - Processes all subdirectories in benchmarks/
# - Skips existing files (resume support)
# - If any conversion fails in a benchmark, skip rest of that benchmark
# - Creates one output directory per benchmark
#
# Usage:
...
CONVERTER="$SCRIPT_DIR/onnx_to_mps.py"

# Check if virtual environment is activated
if [ -z "$VIRTUAL_ENV" ]; then
    echo -e "${YELLOW}Warning: Virtual environment not activated${NC}"
    echo "Consider running: source .venv-gurobi/bin/activate"
    echo ""
fi

# Verify converter script exists
if [ ! -f "$CONVERTER" ]; then
    echo -e "${RED}Error: Converter script not found: $CONVERTER${NC}"
    exit 1
fi
